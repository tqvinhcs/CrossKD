...
from model.loss import RankDistilLoss
...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model, loss, metrics, optimizer, config, data_loaders,
                 ...
                 include_optim_in_ckpts, force_cpu_val, distil_loss, distil_params, ranking_loss, ranking_params, teacher_rank, teacher_params, cache_targets=set(),
        ...
        self.cap_loss = RankDistilLoss(temp=self.ranking_params['cap_t'])
        self.vid_loss = RankDistilLoss(temp=self.ranking_params['vid_t'])
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, minibatch in enumerate(self.data_loaders["train"]):
            ...
            with torch.no_grad():
                ...
                for t in distil:
                    ...
                    for new_mod in distil[t]:
                        ...
                        tmp_sim = torch.matmul(distil_text[t][new_mod].view(-1, distil_text[t][new_mod].shape[-1]),
                                               distil[t][new_mod].t())
            ...
            if self.ranking_loss:
                # print('3 ranking_loss')
                if self.ranking_params['cap_w'] > 0:
                    # print('3.1 caption_loss')
                    loss += self.ranking_params['cap_w'] * self.cap_loss(output["cross_view_conf_matrix"],
                                                                         output["cross_view_text_matrix"])

                if self.ranking_params['vid_w'] > 0:
                    # print('3.2 video_loss')
                    loss += self.ranking_params['vid_w'] * self.vid_loss(output["cross_view_conf_matrix"].t(),
                                                                         output["cross_view_vid_matrix"])
