...
class CEModule(nn.Module):
    ...
    def forward(self, text, experts, ind, raw_captions):
        ...
        if self.task == "classification":
            ...
            cross_view_conf_matrix, cross_view_text_matrix, cross_view_vid_matrix = sharded_cross_view_inner_product(
                ind=ind,
                vid_embds=experts,
        ...
        return {
            ...
            "cross_view_text_matrix": cross_view_text_matrix,
            "cross_view_vid_matrix": cross_view_vid_matrix,
            "text_embds": text_embd,
            "vid_embds": experts,
        }
...
def sharded_cross_view_inner_product(vid_embds, text_embds, text_weights,
    ...
    for idx, modality in enumerate(subspaces):
        ...
        text = text_embd_.data.cpu().numpy()
        vids = vid_embd_.data.cpu().numpy()
        # print(msg, (text_embd_**2).sum(), (vid_embd_**2).sum())
        weighting = text_weight_tensor[:, :, idx]
        sims += weighting * th.matmul(text_embd_, vid_embd_.t())  # (T x num_caps) x (B)

    ####
    text_sims = th.zeros(T * num_caps, T * num_caps, device=device)
    for idx, modality in enumerate(subspaces):
        text_embd_ = text_embds[modality].view(T * num_caps, -1)
        msg = "expected weights to be applied to text embeddings"
        assert text_embd_.shape[0] == text_weights.shape[0], msg
        text_embd_ = text_embd_ / l2_mass_text
        weighting = text_weight_tensor[:, :, idx]
        if num_caps == 1 and B == T * num_caps:
            text_sims += weighting * th.matmul(text_embd_, text_embd_.t())  # (T x num_caps) x (T x num_caps)
        else:
            text_sims += th.matmul(text_embd_, text_embd_.t())  # (T x num_caps) x (T x num_caps)
    ####
    vid_sims = th.zeros(B, B, device=device)
    for idx, modality in enumerate(subspaces):
        vid_embd_ = vid_embds[modality].reshape(B, -1) / l2_mass_vid
        msg = "expected weights to be applied to text embeddings"
        assert text_embd_.shape[0] == text_weights.shape[0], msg
        weighting = text_weight_tensor[:, :, idx]
        if num_caps == 1 and B == T * num_caps:
            vid_sims += weighting * th.matmul(vid_embd_, vid_embd_.t())  # (T x num_caps) x (B)
        else:
            vid_sims += th.matmul(vid_embd_, vid_embd_.t())  # (T x num_caps) x (B)
    ...
    if num_caps > 1:
        ...
        if merge_caption_similiarities == "avg":
            ...
            text_sims = text_sims.view(B, num_caps, B)
            # vis = False
            # if vis:
            #     import sys
