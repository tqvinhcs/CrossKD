...
def run_exp(config):
    ...
    for ii, seed in enumerate(seeds):
        ...
        trainer = Trainer(
            ...
            distil_params=config.get("distil_params", None),
            ranking_loss=config.get("ranking_loss", False),
            ranking_params=config.get("ranking_params", None),
            teacher_rank=config.get("teacher_rank", False),
...
def main():
    ...
    args.add_argument('--config', help='config file path', default="configs_teachtext/msvd-train-gpt2-xl-finetuned-mte-adam-kld.json")
    ...
    args.add_argument('--device', help="indices of GPUs to enable", default=0)
